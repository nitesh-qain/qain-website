...
- [x] `[AUTO]` Basic Core Web Vitals pass — check the animated gradient/glow CSS isn't hurting LCP/CLS on mobile. — done 2026-09-23, audit only, no code change needed (see `seo/reports/2026-09-23.md` for the measurements)
- [x] `[AUTO]` Fix mobile horizontal overflow (unrelated to the gradient/glow, found during the 2026-09-23 CWV audit): on `index.html` the footer `.foot-links` row doesn't wrap on a 375px viewport (672px wide, pushes the page to 693px, causes horizontal scroll); on `case-studies/automation.html` the `.data-table` doesn't fit either (430px wide, one column alone is 106px). Needs `flex-wrap`/responsive-table handling, not a glow/gradient fix. — done 2026-09-24, commit `002d019`
...
- [x] `[AUTO]` Build `/services/` hub + dedicated service pages, in the keyword-driven order above — not the original "4 pages named after the 4 homepage cards" plan. Hub targets `qa outsourcing` / `software testing services`; first two dedicated pages target `test automation services` (the Loopsy/AI automation angle) and `manual testing services`. — done 2026-09-25, commit `1f2cdb1` (hub + both Tier-1 dedicated pages shipped; the `hire qa engineers` / Dedicated QA Engineers page is intentionally deferred — still needs the owner's confirm on folding it into `/contract-to-hire/` vs. a standalone page noted in this task originally, so it's left off PLAN.md as a new `[NEEDS-YOU]` item below rather than guessed at)
- [ ] `[NEEDS-YOU]` Confirm whether the `hire qa engineers` keyword (Tier 2) should fold into `/contract-to-hire/` (adding on-page copy/heading targeting that phrase) or get its own `/services/dedicated-qa-engineers/` page — the original Phase 2 hub task flagged this as needing an owner decision before building, and it's still open.
- [ ] `[NEEDS-YOU]` If QAInfinity or its engineers hold any real, verifiable certifications (ISTQB, ISO, specific vendor certs, etc.), share the details/proof so `/about` can list them by name. None currently appear anywhere on the site, so the 2026-09-26 `/about` build deliberately did not invent any — it framed "credentials" as track record (7+ years, 100% contract-to-hire conversion, 98% client satisfaction) and the real training/vetting process instead.
- [x] `[AUTO]` Build a real `/about` page — team credentials, certifications, and explicit time-zone-overlap messaging for UK/AU/APAC buyers. — done 2026-09-26, commit `479b143` (credentials framed as track record/vetting/training process — no specific certifications like ISTQB/ISO exist anywhere on the site to draw on, so none were invented; see the 2026-09-26 report for detail and a new `[NEEDS-YOU]` flag below if the owner wants named certifications added)
...
- 2026-09-23 — Phase 1: Core Web Vitals audit of the hero/spotlight gradient-glow CSS across all 7 pages (headless Chromium, 375x812 mobile viewport, 4x CPU throttle). Found no `@keyframes`/`animation`/`requestAnimationFrame`/canvas anywhere in the codebase — the "glow" is a single static `radial-gradient` painted once via an absolutely-positioned `::before` (z-index -1, taken out of normal flow), not an animated effect. Measured CLS = 0 and LCP 428–768ms (LCP element was the hero `<p class="lede">` text, not the gradient) on every page — both comfortably within Google's "good" thresholds even under throttling. No code change needed; closing the task as verified-clean. See `seo/reports/2026-09-23.md` for full per-page numbers
- 2026-09-24 — Phase 1: fixed mobile horizontal overflow found in the 2026-09-23 CWV audit — `index.html` `.foot-links` now wraps (`flex-wrap`) instead of forcing a 672px-wide row on a 375px viewport, and `case-studies/automation.html`'s 4-column `.data-table` is now wrapped in a horizontally-scrollable `.table-scroll` container instead of overflowing the page. Verified headless Chromium at 375x812 on all 8 site pages: `document.scrollWidth === document.clientWidth` (no horizontal overflow) everywhere, all pages return HTTP 200, internal links resolve. Console showed only pre-existing sandbox-proxy failures fetching `googletagmanager.com`/`fonts.googleapis.com` (unrelated to this change, present on every page before and after). — commit `002d019`
- 2026-09-25 — Phase 2: built `/services/` hub (targets `qa outsourcing` / `software testing services`) plus two Tier-1 dedicated pages, `/services/test-automation/` (targets `test automation services`, the Loopsy/agentic-QA angle) and `/services/manual-testing/` (targets `manual testing services`) — each with its own `Service` JSON-LD, canonical URL, and the standard Book-a-Consult + Release-Readiness closing CTA. Added "Learn more" links from the two relevant homepage service cards and a "View all services" link in the services section head; added all 3 new URLs to `sitemap.xml`. Content reuses only real, already-published facts (Angara's 8hrs→30min regression result, the 11/11 Contract-to-Hire placement stat, Loopsy's up-to-80%-faster stat, the 5–10 business day placement window, NDA/IP and time-zone-overlap language) — no new stats, client names, or testimonials invented. `hire qa engineers` / Dedicated QA Engineers page deliberately deferred — flagged as a new `[NEEDS-YOU]` in Phase 2 above since it needs an owner decision on whether it folds into `/contract-to-hire/` or gets its own page. Verified with headless Chromium (Playwright, installed fresh into the sandbox this run) at 375x812 across all 10 site pages: HTTP 200, zero dead internal links, zero horizontal overflow, nav + CTA present on every page, and all JSON-LD blocks parse as valid JSON; also spot-checked both new dedicated pages visually via full-page screenshots at desktop (1280px) and mobile (375px) widths. — commit `1f2cdb1`
- 2026-09-26 — Phase 2: built `/about` page (`AboutPage` + `Organization` JSON-LD, canonical URL, standard closing CTA). Content: positioning ("pure-play" partner for UK/Europe/Australia-NZ/APAC buyers), the same 4 homepage proof stats (7+ years, Web·Mobile·API, 100% contract-to-hire conversion, 98% client satisfaction), explicit UK/EU/AU-NZ/APAC time-zone-overlap language (reused from the homepage FAQ and `/services/` page), a "how we build capability" section grounded in the real training/feedback-loop process described in the real-money-gaming case study, and a Loopsy section linking to the homepage spotlight. Deliberately did **not** invent specific certifications (ISTQB/ISO/etc.) since none exist anywhere on the site to draw on — flagged as a new `[NEEDS-YOU]` above in case the owner wants to supply real ones. Linked from the homepage footer and added to `sitemap.xml`; did not touch the sticky nav on any page (kept this a single, low-risk task rather than a sitewide nav change). Verified with headless Chromium (Playwright) at 375x812 across all 11 live site pages: HTTP 200, zero dead internal links, zero horizontal overflow (caught and fixed one on `/about` itself — a stray negative margin on the stats strip — before shipping), nav + CTA present, all JSON-LD valid; also reviewed full-page screenshots of `/about` at desktop (1280px) and mobile (375px). — commit `479b143`
